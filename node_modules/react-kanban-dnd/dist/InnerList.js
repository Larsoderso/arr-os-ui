...
var InnerList = /** @class */ (function (_super) {
    ...
    InnerList.prototype.render = function () {
        ...
        return idx_1.default(column, function (_) { return _.rows[0]; })
            ? column.rows.map(function (row, index) { return (React.createElement(react_beautiful_dnd_1.Draggable, { key: row.id, draggableId: row.id, index: index }, function (dragProvided, dragSnapshot) { return (React.createElement(Card_1.default, { key: index, row: row, isDragging: dragSnapshot.isDragging, provided: dragProvided, dragSnapshot: dragSnapshot, renderCard: _this.props.renderCard, cardWrapperStyle: cardWrapperStyle })); })); })
            : [];
    };
    return InnerList;
}(React.Component));
